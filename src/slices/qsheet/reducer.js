...
const QsheetsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(deleteQsheetList.fulfilled, (state, action) => {
      state.qsheetLists = state.qsheetLists.filter(
    ...
    builder.addCase(getQsheetLogList.fulfilled, (state, action) => {
      state.qsheetLogList = action.payload.content;
    });
    builder.addCase(getQsheetLogList.rejected, (state, action) => {
      state.error = action.payload.error || null;
    });
  },
});
